...
namespace Nalco_Consumables.Controllers
{
    ...
    public class EmploySearchController : ApiController
    {
        ...
        public object Post(string id, [FromBody] JObject data)
        {
            try
            {
                using (SqlConnection conn = new SqlConnection())
                {
                    ...
                    {
                        SqlCommand command = new SqlCommand("SELECT employ_dept_cd, employ_dept_name FROM nalco_materials.dbo.np_employ WHERE employ_dept_cd LIKE '%" + id + "%' OR employ_dept_name LIKE '%" + id + "%' GROUP BY employ_dept_cd,employ_dept_name;", conn);
                        using (SqlDataReader reader = command.ExecuteReader())
                        {
                            if (reader.HasRows == true)
                            {
                    ...
                    else if ((bool)dataval["dept"] == false)
                    {
                        ...
                        SqlCommand command = new SqlCommand("SELECT employ_loc_cd , employ_loc_name,employ_dept_name FROM nalco_materials.dbo.np_employ WHERE employ_dept_cd=@1 AND (employ_loc_cd LIKE '%" + id + "%' OR employ_loc_name LIKE '%" + id + "%') GROUP BY employ_loc_cd,  employ_loc_name,employ_dept_name;", conn);
